   ...
   void MainWindow::createToolBar ()
    {
      ...
      styleComboBox->setToolTip(tr("选择画笔线型")); //这样能看到按钮用法比较友好
       // 此处在下拉框的每个Item里，设置了一个item的userData，
       // 该用户数据作为用户选择的标志
      styleComboBox->addItem (tr("实线(SolidLine)"), static_cast<int>(Qt::SolidLine));//前面的能看到，后面是看不到的
      ...
      styleComboBox->addItem (tr("连点划线(DashDotDotLine)"), static_cast<int>(Qt::DashDotDotLine));
       connect (styleComboBox, static_cast<void(QComboBox::*)(int)>(&QComboBox::currentIndexChanged), //信号和槽做连接
                this, &MainWindow::penStyleChangged);
       styleComboBox->setCurrentIndex(1);
      // 线宽选择框
      widthLabel = new QLabel(tr("线宽"));
      widthSpinBox = new QSpinBox;  //QSpinBox用
